...
<template>
    <div class="be-dialog" :class="dialogModels" v-show="!isMinus && isShow">
        <div class="be-dialog-container animate__animated"
             ...
             :class="`${animateClass} animate__faster`"
             v-drag = "{isDrag:isDrag}">
            <div class="be-dialog-title">
                <div class="be-dialog-contanter-head" :id="`be-head${this._uid}`">
                    <span class="be-title">{{titles}}</span>
                    <!-- @slot 弹窗头部按钮 -->
                    <slot name="headerIcon">
                        <i class="fa el-icon-close be-icons" @click="close"></i>
                        <i class="fa el-icon-minus be-icons" @click="minusPanel($event)" v-show="isMinus"></i>
                    </slot>
                </div>
            </div>
            <!-- @slot 弹窗主体-->
