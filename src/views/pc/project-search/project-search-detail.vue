      ...
      <div class="base-info">
        ...
        <div class="base-info-item">
          <div style="flex: 1">
            ...
            <span v-if="baseInfo.transactionsTotal" class="total">{{
              numberToCommaString(baseInfo.transactionsTotal)
            }}</span>
            <span v-if="!baseInfo.transactionsTotal" class="total">{{ $t('lang.emptyData') }}</span>
          </div>
          <div style="flex: 1">
            <p>{{ $t('lang.projectExplorer.detail.lastDate') }}</p>
            <p v-if="baseInfo.lastTradeData" class="date">
              {{ formatDate(createDate(baseInfo.lastTradeData)) }}
            </p>
            <p v-if="!baseInfo.lastTradeData" class="date">{{ $t('lang.emptyData') }}</p>
            <p v-if="baseInfo.lastTradeData" class="time">
