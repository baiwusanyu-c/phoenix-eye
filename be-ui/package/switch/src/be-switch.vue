<template>
  <div
    ...
    tabindex="0"
    @click="handleClick">
    <div v-if="!innerState" :class="`be-switch__${size}_slot__unChecked`">
      <slot name="unCheckedRender" :state="innerState" />
    </div>

    <div class="be-switch--circle">
      <be-icon v-if="isLoading" spin icon="loading" custom-class="be-switch--circle--icon" />
    </div>
    <div v-if="innerState" :class="`be-switch__${size}_slot__checked`">
